...
function defaultPackagedAppDir() {
  if (isMacTarget) {
    ...
    return path.join(ROOT, 'release', archDir, 'KoinosOne.app');
  }
  if (isWindowsTarget) {
...
const APP_EXECUTABLE = isMacTarget
  ? path.join(APP_DIR, 'Contents', 'MacOS', 'KoinosOne')
  : path.join(APP_DIR, `KoinosOne${ext}`);
...
function requireTelenoNodeVersion(filePath) {
  if (isWindowsTarget) return;
  if (!fs.existsSync(filePath)) return;

  let output = '';
  try {
    output = execFileSync(filePath, ['--version'], {
      encoding: 'utf8',
      timeout: 10000,
    }).trim();
  } catch (error) {
    fail(`failed to inspect native version for ${path.relative(ROOT, filePath)}: ${error.message}`);
    return;
  }

  if (!versionMatchesSemanticVersion(output, expectedTelenoNodeVersion)) {
    fail(`native version mismatch in ${path.relative(ROOT, filePath)}: expected ${expectedTelenoNodeVersion}, got ${output || '(empty)'}`);
  }
}
  ...
  for (const filePath of requiredFiles) {
    if (filePath === APP_EXECUTABLE || filePath.endsWith(`${path.sep}teleno_node${ext}`)) {
      ...
      if (filePath.endsWith(`${path.sep}teleno_node${ext}`)) {
        ...
        requireTelenoNodeVersion(filePath);
...
console.log(`Packaged app verification passed: ${requiredFiles.length} required files, app ${appSizeMb} MB, Koinos One resources ${resourceSizeMb} MB.`);
