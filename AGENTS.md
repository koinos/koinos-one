...
Immediate critical path is now split between mainnet networking and real external producer validation. Sprint 1.3 is complete locally, and Sprint 2 Phase A private federated validation, Phase B private PoB validation, the 30-minute private PoB soak, and isolated seed-host external validation are complete. Gate F mainnet networking signoff still blocks a mainnet canary: first run `scripts/probe-mainnet-seeds.sh` with `SEED_PROBE_OUTPUT=scripts/mainnet-peer-validated.txt`; if Go cannot find a currently stable Peer RPC-capable peer, do not spend time on a C++ soak. If Go finds a peer, rerun that peer immediately enough times to verify it is not a one-off transient before starting the C++ soak. The remaining Sprint 2 items require a real VHP-staked testnet producer and independent peer acceptance/monitoring.
