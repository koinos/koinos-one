...
The legacy architecture is powerful but operationally heavy. It runs many services, several storage backends, and an AMQP broker to coordinate internal RPC and event traffic. That creates overhead in three places:

- Runtime overhead: frequent internal calls serialize protobuf messages, route through AMQP, cross process boundaries, deserialize responses, and then repeat the same path back to the caller.
- Operational overhead: node startup, shutdown, logging, health checks, configuration, backup restore, and packaging all have to coordinate multiple services.
- Optimization overhead: performance work is split across processes, queues, thread pools, and storage engines, making it harder to tune the node as one system.

The monolith is designed to keep the protocol behavior intact while removing internal complexity that users and block producers do not need to see.
...
The target architecture is a single C++ process with modular internal components:

- `chain` validates blocks and transactions, tracks head/LIB state, and executes contract reads.
- `block_store` persists block records and metadata in the monolith storage layout.
- `transaction_store`, `contract_meta_store`, and optional stores expose the same service semantics as legacy components.
- `mempool` owns pending transaction admission, pending nonce/resource checks, expiration, and pending transaction queries.
- `p2p` owns libp2p networking, Koinos Peer RPC, sync, gossip, seed connectivity, peer snapshots, and gossip production readiness.
- `jsonrpc` exposes the public JSON-RPC API expected by wallets and tools such as `kcli`.
- `grpc` exposes the generated typed `koinos.services.koinos` protobuf service for gRPC clients.
- `block_producer` assembles resource-bounded blocks, signs federated or PoB blocks, handles failed transaction retry/pruning, and submits produced blocks back through the same chain path used for received blocks.

The components keep their responsibilities, but they communicate through direct interfaces, an internal service registry, and typed events instead of AMQP. This preserves the conceptual service boundaries while removing unnecessary process and broker boundaries.
...
- Existing chain data and backup migration semantics.
- PoB/VHP block production rules.
- Mempool, nonce, resource accounting, transaction inclusion, and block validation behavior.
- Deterministic state transition and persistence behavior.

This is why the implementation plan includes legacy JSON-RPC/gRPC comparisons, upstream `koinos-integration-tests` compatibility, private federated and PoB testnets, public testnet block production, live `kcli` transaction validation, and external legacy observer witness checks.
