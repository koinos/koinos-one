...
- [ ] Run and maintain the transaction/operation/performance regression suite against the live monolith producer: client reads, valid transfer smoke, small mempool burst, negative transaction handling, contract reads, independent observer verification, soak-time transaction injection, and bounded performance regression sampling.
