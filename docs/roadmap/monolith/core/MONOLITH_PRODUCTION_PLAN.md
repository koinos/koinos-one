...
- [~] Indexing speed: measure blocks/sec during sync (target: 10,000-15,000 vs 3,600). Live P2P catch-up is measured; the old target still needs an offline/replay benchmark before it can be treated as satisfied.
...
- [~] Verify there are no deadlocks under load. Short isolated matrix passed; keep a longer mixed RPC/submit/sync stress as follow-up.
...
- [~] Verify SIGTERM always produces clean shutdown under load. Short live-sync load proof passed; keep longer mixed RPC/submit/sync stress as a release-gate follow-up.
