...
**2026-05-25 public testnet update:** The external testnet is now a concrete validation target, not a missing dependency. Before attempting a VHP-staked monolith producer, run the monolith as a non-producing observer against `testnet.koinosfoundation.org` using the public testnet chain ID and P2P endpoint. Exit criteria for this observer step are: correct chain ID, successful Peer RPC handshake, connected peer snapshots, head progress through JSON-RPC, and no severe P2P/sync errors. After observer sync is stable, use locally installed `kcli` as the testnet wallet to fund the producer account from the faucet, burn testnet KOIN to VHP, dry-run and submit producer-key registration, then deploy the monolith with that VHP-staked producer key. Use `scripts/external-pob-testnet-signoff.sh`, `kcli producer-dashboard`, or an equivalent independent observer check to prove produced blocks are accepted by the existing network.
...
- [ ] Register the monolith block-producer public key on the testnet PoB contract with `kcli register-producer-key --dry-run` before submitting the real registration transaction.
- [ ] Deploy the monolith on the live external testnet with a VHP-staked producer.
