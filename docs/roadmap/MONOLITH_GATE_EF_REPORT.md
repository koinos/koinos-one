...
The generated `docs/roadmap/MONOLITH_GATE_F_SOAK_REPORT.md` must be reviewed for process exits, stalled head height, unacceptable RSS growth, repeated JSON-RPC timeouts, peer reconnect failures, fork watchdog false positives, and shutdown problems before marking Gate F complete.
