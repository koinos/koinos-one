...
| --- | --- |
| Product name | `KoinosOne` in `electron-builder.yml`. |
| App ID | `io.koinosone.desktop` in `electron-builder.yml`. |
| Product version | `1.0.2` in `package.json`. |
...
`npm run build` runs `scripts/generate-build-info.js`, and
`electron-builder.yml` includes the generated file as packaged
`build-info.json`.
...
| `productVersion` | Version read from `package.json`. |
...
| `package.json` version has a prerelease segment, such as `1.2.3-beta.1` | Uses the first prerelease token, such as `beta`. |
